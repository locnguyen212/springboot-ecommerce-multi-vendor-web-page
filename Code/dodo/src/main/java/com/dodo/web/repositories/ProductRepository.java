...
public interface ProductRepository extends JpaRepository<Product, Integer> {
	...
	@Query("SELECT new com.dodo.web.modelview.ProductView(p.productId, p.productName, p.price, p.stockQuantity, p.productImage, p.status, c.categoryId, c.categoryName, sh.user.userId, sh.shopName, sh.shopLogoPath, pr.discountAmount, pr.startDate, pr.endDate, COALESCE(COUNT(r.comment), 0) AS totalComments, COALESCE(AVG(r.rating), 0) AS averageRating) "
	...
	public Page<Product> findByOwnerId(@Param("ownerId") Integer ownerId, Pageable pageable);
	
	@Query("SELECT p FROM Product p WHERE p.shopowner.id = :ownerId")
	public List<Product> findByOwnerId2(@Param("ownerId") Integer ownerId);

	@Query(value = "SELECT new com.dodo.web.modelview.ProductView(p.productId, p.productName, p.price, p.stockQuantity, p.productImage, p.status, c.categoryId, c.categoryName, sh.user.userId, sh.shopName, sh.shopLogoPath, pr.discountAmount, pr.startDate, pr.endDate, COALESCE(COUNT(r.comment), 0) AS totalComments, COALESCE(AVG(r.rating), 0) AS averageRating) "
			+ "FROM Product p LEFT JOIN p.category c "
			+ "LEFT JOIN p.shopowner sh "
			+ "LEFT JOIN Promotion pr ON p.productId = pr.product.productId "
			+ "LEFT JOIN Review r ON p.productId = r.product.productId "
	...
	@Query("SELECT p FROM Product p WHERE p.shopowner.id = :ownerId AND p.status = :status")
	Page<Product> findByOwnerIdAndStatus(Integer ownerId, Boolean status, Pageable pageable);

	@Query("SELECT p FROM Product p WHERE p.shopowner.id = :ownerId AND p.category.id = :categoryId")
